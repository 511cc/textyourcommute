...
function getSurveyDate() {
  let date;
  if(moment().tz('America/Los_Angeles').isBefore(moment().tz('America/Los_Angeles').hour(AMStartHour))) {
    ...
    if(!introSurvey.answers.length && !introSurvey.status) {
      ...
      DailySurvey.findOne({src: req.body.From, date: date}, (e, dailySurvey) => {
        ...
        dailySurvey.save((e) => {
          ...
          // If it is PM already, send PM survey, else send thank you text
          if(moment().tz('America/Los_Angeles').isAfter(moment().tz('America/Los_Angeles').hour(PMStartHour)) || moment().tz('America/Los_Angeles').isBefore(moment().tz('America/Los_Angeles').hour(AMStartHour))) {
